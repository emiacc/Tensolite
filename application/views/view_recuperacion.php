...
<?php 
	$max = 0;
	$min = 9999999;
	$tot = count($resumenRecuperacion);
	$i = 0;
	echo "<script>var resumen = [";
	foreach ($resumenRecuperacion as $value) {
	...
	echo '];</script>';
	echo '<script>var legend = "Recuperación"; color="dark";</script>';
?>
...
<div id="content" class="content">
		...
		<div class="panel panel-inverse">				
			<div class="panel-body">
				...
				<ul class="list-inline pull-right" style="margin:0;">
					<?php if($data['rol'] < 3 ){ ?>	
					<li><button id="ingreso-modal" type="button" class="btn btn-inverse" data-toggle="modal" data-target="#modal-ingreso">Ingreso Recuperación</button></li>
		...
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title" id="myModalLabel">Ingreso Recuperación</h4>
				</div>
				<div class="modal-body">
					<form class="form-horizontal" action="<?= base_url(); ?>recuperacion/ingreso" method="POST" data-parsley-validate="true">	
						<div class="form-group">
							...
							<div class="col-md-9 ui-sortable">
								<div class="input-group date" id="datepicker-autoClose" data-date-format="dd-mm-yyyy" >
									<input id="inputFechaRecuperacion" name="inputFechaRecuperacion" type="text" class="form-control" value="" placeholder="Seleccione Fecha" data-parsley-required="true" />
	...
	<div class="modal fade" id="modal-filtro">
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-body">
					<form class="form-horizontal" action="<?= base_url(); ?>recuperacion" method="POST" data-parsley-validate="true">
						<div class="form-group">
	...
	<div class="panel panel-inverse">
		<div class="panel-heading">
			...
			<h4 class="panel-title">Recuperación Mensual</h4>
		</div>
		<div class="panel-body">			
	...
	<div class="panel panel-inverse">
		<div class="panel-heading">
			...
			<h4 class="panel-title">Histótico Recuperación</h4>
		</div>
		<div class="panel-body">
			<div class="panel-body">
				<table id="data-table" class="table table-striped table-bordered">
						...
						<tbody>
						<?php 
							foreach ($historico as $recuperacion) 
							{
								$fechaR = new DateTime($recuperacion->fecha);
								
								echo "<tr id='".$recuperacion->id_recuperacion."'>";
								echo "<td>".$recuperacion->id_recuperacion."</td>";
								echo "<td>".date_format($fechaR,'d-m-Y')."</td>";								
								echo "<td>".number_format(($recuperacion->medida/10),1)."</td>";
								echo "<td>".$recuperacion->cantidad."</td>";
								if($data['rol']==1)
								{
...
<div class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" id="modal_delete">
   <div class="modal-dialog modal-sm" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title" id="myModalLabel">Eliminar Recuperación</h4>
      </div>
      <div class="modal-body">
        ¿Está seguro que desea eliminar la recuperación?
      </div>
      <div class="modal-footer">
        <form method="POST" action="<?=base_url()?>recuperacion/eliminar">
...
<script>
	$(document).ready(function() {
		...
		$("#LIrecuperacion").addClass("active");		
		...
		$("#inputFechaRecuperacion").attr("value",currentDay);
	...
	function acciones()
	{
		<?php if($data['rol']==1): ?>
			$(".btn-delete").click(function(){
				$("#id_recuperacion").val($(this).parent().parent().attr("id"));
				$('#modal_delete').modal('show');
			});

			$('#modal_delete').on('hidden.bs.modal', function (){ $("#id_recuperacion").val(""); });
